...
class TestOverviewRoute:
    def test_the_page_has_one_row_per_project_and_one_cell_per_project_per_class(
        ...
        cells = re.findall(r'<td class="c" data-target="cov-([^"]+)"', body)
        assert len(cells) == 3 * len(SOURCE_BADGE_ORDER)
        assert cells[:6] == [f'touch-grass-{c}' for c in SOURCE_BADGE_ORDER]
        for target in cells:
            assert f'<details id="cov-{target}">' in body
    ...
    def test_the_grid_glyphs_follow_the_legend(self, client, seeded):
        ...
        for cls in ('chain_rpc', 'chain_explorer', 'dex_provider'):
            assert cell('touch-grass', cls).group(2) == '●1<sup>ᶜ</sup>', cls
            assert cell('quiet', cls).group(2) == '●1<sup>ᶜ</sup>', cls
        assert cell('touch-grass', 'web').group(2) == '●2'
        assert cell('touch-grass', 'x').group(2) == '●1'
        assert cell('touch-grass', 'telegram').group(2) == '<span class="n0">○</span>'
        assert cell('zzz', 'web').group(2) == '◐1'
        assert cell('zzz', 'x').group(2) == '<span class="n0">○</span>'
        ...
        assert '● admitted (n) · ◐ candidate (n) · ○ none · ✕ suspended/retired · ᶜ inherited from the chain' in grid
        ...
        assert '<li>no source row reaches this project in this class</li>' in grid
    ...
    def test_a_suspended_source_is_a_cross(self, client, seeded, onchain_repository):
        ...
        onchain_repository.commit()
        grid = _body(client)
        assert '<td class="c" data-target="cov-touch-grass-x" title="x: 1 suspended\nx.com/TouchGrassRWA · suspended by operator">✕</td>' in grid
    ...
    def test_the_rpc_cell_names_a_host_and_no_key(self, client, seeded, monkeypatch):
        body = _body(client)
